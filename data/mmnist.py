...
class MovingMNIST(VideoDataset):
    ...
    def make_dataset(cls, data_dir, nx, seq_len, max_speed, deterministic, num_digits, train):
        ...
        if train == 'train':
            ...
            dataset = np.load(os.path.join(data_dir, f'smmnist_val_{num_digits}digits.npz'),
                              allow_pickle=True)
        ...
        return cls(data, nx, seq_len, max_speed, deterministic, num_digits, True if train == 'train' else False)