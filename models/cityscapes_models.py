...
class encoder(nn.Module):
    def __init__(self, dim, nc=3, h=92, w=310, bn=True):
        ...
        self.c5 = nn.Sequential(
            conv(192, 256, act='leaky_relu', stride=2, bn=bn),
            conv(256, 256, act='leaky_relu', bn=bn),
            conv(256, 256, act='leaky_relu', bn=bn)
        )
        # 4x4
...
class decoder(nn.Module):
    ...
    def forward(self, inp):
        ...
        up2 = F.interpolate(d2, size=(8, 16), mode='bilinear', align_corners=True)  # self.up(d1) # 4 -> 8
        d3 = self.upc3(torch.cat([up2, skips[3]], 1))  # 16 x 16
        up3 = F.interpolate(d3, size=(16, 32), mode='bilinear', align_corners=True)  # self.up(d1) # 4 -> 8
        d4 = self.upc4(torch.cat([up3, skips[2]], 1))  # 32 x 32
        up4 = F.interpolate(d4, size=(32, 64), mode='bilinear', align_corners=True)  # self.up(d1) # 4 -> 8
        d5 = self.upc5(torch.cat([up4, skips[1]], 1))  # 64 x 64
        up5 = F.interpolate(d5, size=(64, 128), mode='bilinear', align_corners=True)  # self.up(d1) # 4 -> 8
        d6 = self.upc6(torch.cat([up5, skips[0]], 1))
        up6 = F.interpolate(d6, size=(128, 256), mode='bilinear', align_corners=True)  # self.up(d1) # 4 -> 8
        out = self.upc7(up6)
        return out
